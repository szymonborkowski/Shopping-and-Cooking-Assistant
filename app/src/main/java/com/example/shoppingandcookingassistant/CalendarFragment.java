...
public class CalendarFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
            ...
            ActivityResultLauncher<Intent> activityResultLauncher = registerForActivityResult(
                    ...
                    new ActivityResultCallback<ActivityResult>() {
                        ...
                        public void onActivityResult(ActivityResult result) {
                            ...
                            ArrayList<String> ingredientNamesBuff = intent.getStringArrayListExtra(
                                    AddRecipeEvent.CHOSEN_RECIPE_INGREDIENT_NAMES);
                            ArrayList<String> ingredientAmountsBuff = intent.getStringArrayListExtra(
                                    AddRecipeEvent.CHOSEN_RECIPE_INGREDIENT_AMOUNTS);

                            ingredientsList = new ArrayList<>();
                            for(int i = 0; i < ingredientNamesBuff.size(); i++) {
                                ingredientsList.add(new Ingredient(ingredientNamesBuff.get(i),
                                        ingredientAmountsBuff.get(i)));
                            }
        ...
        rvAdapter = new PlannedMealsRVAdapter(getActivity(), names, portions, daysLeft,
                plannedMealsForDate, date, ingredientsForEachMeal);
        ...
        // open an activity
        addShoppingListBtn.setOnClickListener(this::showPopupWindow);
    ...
    public void showPopupWindow(final View view) {
        ...
        closeBtn.setOnClickListener(view1 -> popupWindow.dismiss());

        // Add shopping list:
        Button saveShoppingListBtn = popupView.findViewById(R.id.saveShoppingListBtn);
        saveShoppingListBtn.setOnClickListener(view12 -> {
            ...
            for (int i = 0; i <= chosenVal; i++) {
                ...
                if (plannedMealsForDate.containsKey(date)) {
                    // parse ingredients
                    // The below lines of code check if an ingredient is already in the shopping
                    // list and instead of placing the same item in twice, the quantity is updated
                    // instead.
                    for(Meal meal : Objects.requireNonNull(plannedMealsForDate.get(date))) {
                        for(Ingredient ingredient : meal.getIngredients()) {
                            if(names.contains(ingredient.getName())) {
                                ingredientsForAllMeals.get(
                                        names.indexOf(
                                                ingredient.getName())).increaseAmount(
                                                        ingredient.getAmountInt());
                            } else {
    ...
    public void saveMap() {
        SharedPreferences preferences = getContext().getSharedPreferences(
                "com.example.shoppingandcookingassistant.MEALS_MAP", Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();
        Gson gson = new Gson();

        // main loop to iterate over each saved date
        int i = 0;
        editor.putInt(LogInScreenActivity.LOGGED_IN_USER_ID + "_date_amount",
                plannedMealsForDate.size());  // how many dates
        for(Map.Entry<String, ArrayList<Meal>> entry : plannedMealsForDate.entrySet()) {  // each date in map
            String dateKey = LogInScreenActivity.LOGGED_IN_USER_ID + "_date_" + i++;
            editor.putString(dateKey, entry.getKey());
            int j = 0;
    ...
    public void loadMap() {
        ...
        int amountOfDates = preferences.getInt(LogInScreenActivity.LOGGED_IN_USER_ID
                + "_date_amount", 0);
        for(int i = 0; i < amountOfDates; i++) {
            String buffDate = preferences.getString(LogInScreenActivity.LOGGED_IN_USER_ID
                    + "_date_" + i, "");
            int amountOfMealsForDate = preferences.getInt(LogInScreenActivity.LOGGED_IN_USER_ID
                    + "_date_" + i + "_meal_amount", 0);
            ArrayList<Meal> buffMeals = new ArrayList<>();
            for(int j = 0; j < amountOfMealsForDate; j++) {
                String jsonMeal = preferences.getString(LogInScreenActivity.LOGGED_IN_USER_ID
                        + "_date_" + i + "_meal_" + j, "");
